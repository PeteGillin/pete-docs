...
I believe that such a law would imply the stronger form of Newton's first law, and would therefore restore determinism in all cases where the stronger first law does so, including on Norton's dome. I also believe that it would outlaw the class of solutions where the particle spontaneously de-orbits in the bowl examples, and so restore determinism there. I strongly suspect that it would be enough to guarantee determinism in all reasonable cases. Now is this law reasonable? Might we not argue that requiring continuity in our solutions when we don't have it in our boundary conditions is perverse? Perhaps. Since we're dealing with hypotheticals here, we can't give a concrete answer.
